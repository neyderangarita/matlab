function [dilatar] = marcas_tusaber3(img1)
...
clear centroideY pepe propied ind tmp n Ne L
 ...
 for n=1:3 
    if n==1 
     ...
     elseif n==2      
        xmin = ImagenRecortar(n-1).BoundingBox(1) + ImagenRecortar(n-1).BoundingBox(4) - 10;
        ymin = ImagenRecortar(n-1).BoundingBox(2) + (ImagenRecortar(n-1).BoundingBox(4) * 2) - 5;
        ancho = ImagenRecortar(n+3).BoundingBox(1) - ImagenRecortar(n-1).BoundingBox(1) - 14;  
